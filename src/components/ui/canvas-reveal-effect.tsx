...
const ShaderMaterial = ({ source, uniforms, maxFps = 60 }: { source: string; uniforms: Uniforms; maxFps?: number }) => {
  ...
  const getUniforms = () => {
    ...
    for (const k in uniforms) {
      ...
      switch (u.type) {
        case "uniform1f": prepared[k] = { value: u.value, type: "1f" }; break;
        case "uniform1fv": prepared[k] = { value: u.value, type: "1fv" }; break;
        case "uniform3fv": prepared[k] = { value: u.value.map((v: number[]) => new THREE.Vector3().fromArray(v)), type: "3fv" }; break;
        default: break;
      }
    }
